...
- DB schema: `lib/db/src/schema/*.ts` (tables: candidates, candidate_positions, candidate_records, candidate_record_enrichment, candidate_donor_categories, candidate_donor_signals, candidate_vote_signals, issues, questions, voters, voter_answers, voter_stances, sync_meta)
- House roll-call votes: pure aggregation in `artifacts/api-server/src/lib/votes.ts` (`isSubstantivePassageVote`, `isSubstantiveLegislationType`, `aggregateVoteSignals`); Congress fetch helpers `fetchHouseVoteList`/`fetchHouseVoteDetail`/`fetchHouseVoteMembers` + `billIssueAndDirection` in `src/lib/congress.ts`; sync in `src/scripts/syncVotes.ts`; stored in `candidate_vote_signals` (position, voteCount, agreeShare, example votes jsonb); blended into positions at read time by `applyVoteEvidence` in `src/lib/matching.ts`
...
- House members get a THIRD evidence layer: ACTUAL roll-call votes. Unlike donor signals (which only adjust confidence), votes MOVE positions — `applyVoteEvidence` blends the vote-derived position into the sponsorship/party base at read time (weight scales with vote volume, capped ~0.7) and raises confidence. Senate has no roll-call ingest, so it stays sponsorship-derived. Only substantive passage votes on real legislation (HR/S/HJRES/SJRES) count — procedural and rule-adoption votes are excluded.
